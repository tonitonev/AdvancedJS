...
(function (eventSystem) {
    ...
    var halls = eventSystem.halls;
    //select.options.length = halls.length;

    for(index in halls) {
        select.options[select.options.length] = new Option(halls[index]._name, halls[index]._name);
    }
    //var optionsString = "";
    //for (var i= 0; i<halls.length; i++) {
    //    var hallName = halls[i]._name;
    //        optionsString += "<option value=" + hallName + ">" +  + "</option>";
    //    select.value
    //}
    //select.value = eventSystem.halls[0]._name;
